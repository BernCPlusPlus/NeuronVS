...
#include "../../1_driver/mac/RemoteDriverMac.h"
...
//-------------------------------------------------------------------------------------------------------
extern RemoteDriverPool *gstRemoteDriverPool;
...
AudioEffect::AudioEffect (audioMasterCallback audioMaster, long numPrograms, long numParams)
{
	...
	if (gNumberOfInstances>0)
		RaiseThreadSafeAlert(0, CFSTR("Warning"), CFSTR("Running more than one instance of the NeuronVS in can cause crashes and inconsistent behaviour of the <NUKE> controller"), CFSTR("OK"), NULL, NULL);
	
	gNumberOfInstances++;
	
	if (!gstRemoteDriverPool) {
//		RaiseThreadSafeAlert(0, CFSTR("!gstRemoteDriverPool"), CFSTR("Yess"), CFSTR("OK"), NULL, NULL);
		gstRemoteDriverPool = new RemoteDriverPool;
//		printf("new RemoteDriverPool\n");
	}
...
AudioEffect::~AudioEffect ()
{
	...
	if (gstRemoteDriverPool) {
//		printf("delete RemoteDriverPool\n");

		delete gstRemoteDriverPool;
		gstRemoteDriverPool = NULL;
	}
